...
`;
        ...
        notifyDesktopPetQQInMessage(userContent);
            ...
            dispatcherOptions: {
              ...
              deliver: async (payload: { text?: string; mediaUrls?: string[]; mediaUrl?: string }, info: { kind: string }) => {
                ...
                if (mediaTagMatches.length > 0) {
                  ...
                  const sendQueueSummary = sendQueue
                    .map((i) => (i.type === "text" ? i.content : `${i.type}:${i.content}`))
                    .join(" | ");
                  notifyDesktopPetQQOutbound(sendQueueSummary);
                  ...
                  if (payloadResult.payload) {
                    ...
                    if (isCronReminderPayload(parsedPayload)) {
                      ...
                      try {
                        ...
                        notifyDesktopPetQQOutbound(confirmText);
                      } catch (err) {
                      ...
                      notifyDesktopPetQQOutbound(
                        `[结构化载荷:${parsedPayload.type}:${parsedPayload.mediaType}] ${parsedPayload.path ?? ""}${parsedPayload.caption ? ` | ${parsedPayload.caption}` : ""}`,
                      );
                      // 记录活动并返回
                      pluginRuntime.channel.activity.record({
                        channel: "qqbot",
...

                {
                  const outboundDesc =
                    imageUrls.length > 0
                      ? `${replyText.slice(0, 600)}${imageUrls.length ? ` [图片×${imageUrls.length}]` : ""}`
                      : replyText;
                  notifyDesktopPetQQOutbound(outboundDesc);
                }
